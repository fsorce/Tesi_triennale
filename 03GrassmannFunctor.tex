...
If we interpret $\Bc_{k,n}$ and $\K[X_{k,n}]$ as the coordinate rings of the affine spaces $\bigwedge^k\K^n$ and $\Mc(k,n)$ respectively, then it is clear by construction that the map induced between them from $\phi^\#$ is $A\mapsto \sum_{I\in\omega(k,n)}\det A_I e_I$, which we already know as $\phi^s$.
\end{remark}

\begin{proposition} $\ker\phi^\#$ is a homogeneous prime ideal which does not contain all of the $z_I$.
\end{proposition}
\begin{proof}
...
Applying the functor $\eta$  defined during the proof of proposition (\ref{GrIRepresentGrIFunctors}), which we will denote $\eta^I$ to emphasize which determinant we consider at the denominator, we obtain 
