...
To get an idea of why the second condition we cite the following
\begin{proposition}
...
Suppose now that the $f_i$ are isomorphisms. Let $f$ be the morphism $F\to G$ obtained as above and let $g:G\to F$ be the morphism obtained the same way but from the morphisms $f_i\ii:G_i\to F_i$. It is easy to see that the restrictions of $f\circ g$ (respectively  $g\circ f$) to the $G_i$ (respectively $F_i$) and the double intersections yield the identity, so the compositions themselves must glue to the identity of $F$ and $G$.
\end{proof}
...
Using the openness of $F_j$ we find $U_{ij}\subseteq X_i$ which represents $h_{X_i}\times_FF_j\cong F_{i,j}$. By uniqueness of moduli spaces we see that there exists an isomorphism $\vp_{ji}:U_{ij}\to U_{ji}$, which we can choose in such a way as to translate to the identity transformation $F_{i,j}= F_{j,i}$.

Our choice for the maps $\vp_{ji}$ makes the cocycle condition $\vp_{ki}=\vp_{kj}\circ \vp_{ji}$ hold. We can thus glue the $X_i$ to a scheme $X$. Since $\xi_i\res{U_ij}=\xi_j\res{U_{ij}}$ by construction of $\vp_{ji}$, we find a family $\xi\in F(X)$ by the sheaf property of $F$. It follows easily that $(X,\xi)$ represents $F$.
