...
\item If $A$ and $B$ are objects in a category (respectively categories / functors) then $\Hom(A,B)$ denotes the set of morphisms (respectively functors / natural transformations) from $A$ to $B$.

In the first case, if $A,B\in\Cc$ we sometimes write $\Hom_\Cc(A,B)$ to emphasize the category in which we are considering the morphisms.
\item The notation $\Fun(\Cc,\Dc)$ denotes the category of functors from $\Cc$ to $\Dc$, with morphisms being natural transformations.
\end{itemize}
...
\[h_X:\functorDef{\Cc\op}{\Set}{T}{\Hom(T,X)}{f:T\to S}{h_X(f):\Hom(S,X)\to \Hom(T,X)}\]
where, if $g:S\to X$ then $h_X(f)(g)=g\circ f:T\to X$.
\end{definition}
\begin{remark}
...
where $\sim$ refers to an equivalence relation imposed on the objects we are studying.
\end{remark}
...
\[\begin{tikzcd}
	...
	& {F(T)} & {F(S)} \\
	{F(X)}
	\arrow["{h_X(f)}"', from=1-1, to=2-2]
	\arrow["{h_X(f\circ g)}", from=1-1, to=2-3]
	\arrow["{F(\cdot)(u)}"', from=1-1, to=4-1]
	\arrow["{h_X(g)}"', from=2-2, to=2-3]
...
\begin{remark}
If $X=Y$ then $h_{id_X}=id_{h_X}$. Indeed if $f:T\to X$ then
\[(h_{id_X})_T(f)=id_X\circ f=f=(id_{h_X})_T(f).\]
\end{remark}
...
\end{remark}
\begin{proof}
If $\vp:h_A\to h_B$ is an isomorphism with inverse $\vp'$, by the Yoneda lemma they coorespond to maps $f:A\to B$ and $f':B\to A$, which must be inverses because the Yoneda embedding is fully faithful.
...
\begin{remark}
Using the Yoneda lemma (\ref{YonedaLemma}) we may equivalently require the following diagram to be an equalizer
% https://q.uiver.app/#q=WzAsMyxbMCwwLCJcXEhvbShoX1gsRikiXSxbMiwwLCJcXGRpc3BsYXlzdHlsZSBcXHByb2RfayBcXEhvbShoX3tVX2t9LEYpIl0sWzQsMCwiXFxkaXNwbGF5c3R5bGUgXFxwcm9kX3tpLGp9XFxIb20oaF97VV9pXFxjYXAgVV9qfSxGKSJdLFswLDFdLFsxLDIsIiIsMCx7Im9mZnNldCI6LTF9XSxbMSwyLCIiLDIseyJvZmZzZXQiOjF9XV0=
\[\begin{tikzcd}
...
\end{itemize}
both of which are well known properties of maps and scheme morphisms respectively.
\end{proof}
...
\end{definition}
Like before we can rephrase this definition in terms of families


\begin{definition}[Open cover of a functor v.2]
Let $F:\Psh S\Set$ be a moduli problem. A collection of open subfunctors $F_i\subseteq F$ forms an open cover of $F$ if for any $S$-scheme $T$ and any family $\xi\in F(T)$, the subschemes $U_i\subseteq T$ determined by the openness of the $F_i$ form a cover of $T$.
\end{definition}
\begin{remark}
...
Let $F$ and $G$ be Zariski sheaves, $\cpa{F_i\to F}$ and $\cpa{G_i\to G}$ be open covers and $f_i:F_i\to G_i$ be natural transformations such that $f_i\res{F_i\cap F_j}=f_j\res{F_i\cap F_j}$. Then there exists a natural transformation $f:F\to G$ which restricts to $f_i$ on $F_i$.

Moreover, if each $f_i$ is an isomorphism then $f$ too is an isomorphism.
\end{proposition}
\begin{proof}
...
% https://q.uiver.app/#q=
\[\begin{tikzcd}
	{h_{U_i}} & {F_i} & {G_i} \\
	{h_T} & F & G
	\arrow["{\zeta\circ h_{\iota_i}}", from=1-1, to=1-2]
...
\end{tikzcd}\]
Let $g_i=f_i\circ \zeta\circ h_{\iota_i}$ and note that
\[g_i\res{h_{U_i\cap U_j}}=f_i\res{F_i\cap F_j}\circ \zeta\circ h_{\iota_i\res{U_i\cap U_j}}=f_j\res{F_i\cap F_j}\circ \zeta\circ h_{\iota_j\res{U_i\cap U_j}}=g_j\res{h_{U_i\cap U_j}}.\]
Because $G$ is a Zariski sheaf, there exists $\zeta':h_T\to G$ such that $\zeta'\circ h_{\iota_i}=g_i$. Because of the Yoneda lemma (\ref{YonedaLemma}), we have constructed a map $F(T)\to G(T)$ which is functorial in $T$, i.e. $f:F\to G$. By construction it is also clear that $f\res{F_i}=f_i$.\bigskip

Suppose now that the $f_i$ are isomorphisms. Let $f$ be the morphism $F\to G$ obtained as above and let $g:G\to F$ be the morphism obtained the same way but from the morphisms $f_i\ii:G_i\to F_i$. By checking the restrictions to a cover and double intersections, it is easy to see that $f$ and $g$ are inverses.
\end{proof}
...
\end{theorem}
\begin{proof}
For this proof we will mainly follow the version presented in \cite{GortzAlgGeoISchemes} (Theorem 8.9, page 212).\bigskip
...
To finish the proof we need to verify that $(X,\xi)$ is a fine moduli space for $F$, which is the case because of proposition (\ref{MapGluingForZariskiSheaves}) applied to $h_X$ and $F$ where we consider the open covers $\cpa{h_{X_i}\to h_X}$ and $\cpa{F_i\to F}$.\end{proof}
\begin{remark}
